...
#FlexValue
    ...
    if flexTierValue > soloTierValue :
        ...
        realRanked = tierToPrint.capitalize()
        tierToPrint = tierToPrint + "_"
        tierToPrint = (tierToPrint) + (flexRank)
        realRanked += " " + soloRank + " " + str(flexLp) + "LP"
        tierToPrint = tierToPrint + ".png"
        secondRankToPrint ="Rang solo Q"
        secondValue = soloTierValue
        if secondValue != -1 :
            secondRank = soloTier.capitalize()
            secondRank += " " + soloRank + " " + str(soloLp) + "LP"
            secondWins = soloWin
            secondLooses = soloLoose
            secondTotalGame = secondWins + secondLooses
            secondTotalGame = secondWins/ secondTotalGame 
            secondTotalGame = secondTotalGame * 100
            secondTotalGame = str(round(secondTotalGame, 2))
        ...
        realRanked += " " + soloRank + " " + str(soloLp) + "LP"
        tierToPrint = tierToPrint + ".png"
        secondRankToPrint ="Rang flex Q"
        secondValue = flexTierValue
        if secondValue != -1 :
            secondRank = flexTier.capitalize()
            secondRank += " " + flexRank + " " + str(flexLp) + "LP"
            secondWins = flexWin
            secondLooses = flexLoose
            secondTotalGame = secondWins + secondLooses
            secondTotalGame = secondWins/ secondTotalGame 
            secondTotalGame = secondTotalGame * 100
            secondTotalGame = str(round(secondTotalGame, 2))
    embed = discord.Embed(title = blaze, description =("%s\n%s\n %sW/%s L \n %s%s " % (ranktoprint, realRanked,  winstoprint,  loosestoprint, gametotaltoprint, percentsign)))
    file = discord.File(f'./img/{tierToPrint}', filename="image.png")
    embed.set_thumbnail(url="attachment://image.png")
    if secondValue != -1 :
        embed.add_field(name = secondRankToPrint, value = ("%s \n %sW/%sL \n %s %s " % (secondRank, secondWins,  secondLooses,  secondTotalGame, percentsign)))
